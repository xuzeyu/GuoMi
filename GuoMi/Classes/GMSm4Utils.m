...
+ (nullable NSData *)ecbEncryptData:(NSData *)plainData key:(NSString *)key isNeedHexKey:(BOOL)isNeedHexKey {
    if (plainData.length == 0 || key.length % SM4_BLOCK_SIZE != 0) {
        return nil;
    }
    ...
    uint8_t *k_text = isNeedHexKey ? (uint8_t *)[GMUtils hexToData:key].bytes : (uint8_t *)key.UTF8String;
    SM4_KEY sm4Key;
...
+ (nullable NSString *)ecbEncryptText:(NSString *)plaintext key:(NSString *)key isNeedHexKey:(BOOL)isNeedHexKey {
    if (plaintext.length == 0 || key.length % SM4_BLOCK_SIZE != 0) {
        return nil;
    }
    
    NSData *plainData = [plaintext dataUsingEncoding:NSUTF8StringEncoding];
    NSData *cipherData = [self ecbEncryptData:plainData key:key isNeedHexKey:isNeedHexKey];
...
+ (nullable NSData *)ecbDecryptData:(NSData *)cipherData key:(NSString *)key isNeedHexKey:(BOOL)isNeedHexKey{
    if (cipherData.length == 0 || key.length % SM4_BLOCK_SIZE != 0) {
        return nil;
    }
...
+ (nullable NSString *)ecbDecryptText:(NSString *)ciphertext key:(NSString *)key isNeedHexKey:(BOOL)isNeedHexKey{
    if (ciphertext.length == 0 || key.length % SM4_BLOCK_SIZE != 0) {
        return nil;
    }
    
    NSData *cipherData = [GMUtils hexToData:ciphertext];
    NSData *plainData = [self ecbDecryptData:cipherData key:key isNeedHexKey:isNeedHexKey];
...
+ (nullable NSData *)cbcEncryptData:(NSData *)plainData key:(NSString *)key isNeedHexKey:(BOOL)isNeedHexKey IV:(NSString *)ivec{
    if (plainData.length == 0 || key.length % SM4_BLOCK_SIZE != 0 || ivec.length != SM4_BLOCK_SIZE * 2) {
        return nil;
    }
...
+ (nullable NSString *)cbcEncryptText:(NSString *)plaintext key:(NSString *)key isNeedHexKey:(BOOL)isNeedHexKey IV:(NSString *)ivec{
    if (plaintext.length == 0 || key.length % SM4_BLOCK_SIZE != 0 || ivec.length != SM4_BLOCK_SIZE * 2) {
        return nil;
    }
    
    NSData *plainData = [plaintext dataUsingEncoding:NSUTF8StringEncoding];
    NSData *cipherData = [self cbcEncryptData:plainData key:key isNeedHexKey:isNeedHexKey IV:ivec];
    NSString *result = [GMUtils dataToHex:cipherData];
...
+ (nullable NSData *)cbcDecryptData:(NSData *)cipherData key:(NSString *)key isNeedHexKey:(BOOL)isNeedHexKey IV:(NSString *)ivec{
    if (cipherData.length == 0 || key.length % SM4_BLOCK_SIZE != 0 || ivec.length != SM4_BLOCK_SIZE * 2) {
        return nil;
    }
...
+ (nullable NSString *)cbcDecryptText:(NSString *)ciphertext key:(NSString *)key isNeedHexKey:(BOOL)isNeedHexKey IV:(NSString *)ivec{
    if (ciphertext.length == 0 || key.length % SM4_BLOCK_SIZE != 0 || ivec.length != SM4_BLOCK_SIZE * 2) {
        return nil;
    }
    
    NSData *cipherData = [GMUtils hexToData:ciphertext];
    NSData *plainData = [self cbcDecryptData:cipherData key:key isNeedHexKey:isNeedHexKey IV:ivec];
    NSString *plaintext = [[NSString alloc]initWithData:plainData encoding:NSUTF8StringEncoding];
